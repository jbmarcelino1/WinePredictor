...
class Trainer(object):
    ...
    def get_estimator(self):
        ...
        if estimator == "xgboost":
            model = XGBClassifier(objective='multi:softmax', n_jobs=self.n_jobs, max_depth=10, learning_rate=0.05,
                                 gamma=3)
            self.model_params = {
    ...
    def set_pipeline(self):
        ...
        feateng_steps = self.kwargs.get('feateng',['year','weather','description_sentiment','title_length'])
        if memory:
            memory = mkdtemp()
        ...
        pipe_year =  make_pipeline(YearVintageEncoder(title='title'),
                                   SimpleImputer(strategy='median'),
                                   QuantileTransformer())
        pipe_weather = make_pipeline(WeatherEncoder(country='country'),
                                    SimpleImputer(strategy='median'),
                                    QuantileTransformer())
        pipe_sentiment = make_pipeline(DescriptionSentimentEncoder(description='description'),
                                       QuantileTransformer())
        pipe_title_length = make_pipeline(TitleLengthEncoder(taster_name='taster_name',title='title'),
                                       QuantileTransformer())
        pipe_vocab_richness = make_pipeline(VocabRichnessEncoder(description='description'),
                                            QuantileTransformer())
        ...
        self.pipeline = Pipeline(steps=[
            ...
            ('clf', self.get_estimator())], memory=memory)
    ...
    def evaluate(self):
        ...
        if self.split:
            ...
            print(colored("f1 train: {} || rmse val: {}".format(f1_train, f1_val), "blue"))
        else:
